...
class OurTrainingArguments(TrainingArguments):
    ...
    cos_loss_weight: float = field(
        default=1.0, metadata={"help": "Cos similarity loss weight"}
    )

    do_proj: bool = field(
        default=True, metadata={"help": "Boolean value to add MLP"}
    )
    ...
    R_ini: float = field(default=1.0)
    la_R: float = field(default=0.01)
    la_mu: float = field(default=0.01)
    R_eps_weight: float = field(default=1e-8)
...
@dataclass
class VICRegTrainingArguments(TrainingArguments):
    ...
    batch_size:int = field(
        default=512, metadata = {"help": "Integer value to the VICReg"}
    )
    ...
    proj_output_dim: str = field(
        default="8192-8192-8192", metadata={"help": "Projection output dimension"}
    )
...
def main():
    ...
    if model_args.model_name_or_path:
        if "roberta" in model_args.model_name_or_path:
            ...
            if model_args.ssl_type == "simcse":
                ...
                model_class = DistilBertForCorInfoMax
            
            elif model_args.ssl_type == "vicreg":
                model_class = DistilBertForVICReg
            elif model_args.ssl_type == "barlow":
                model_class = DistilBertForBarlow
            else:
                raise ValueError("Unknown ssl_type")
            print(config)
            model = model_class.from_pretrained(
            ...
            if model_args.do_mlm:
                pretrained_model = DistilBertPreTrainedModel.from_pretrained(
                    model_args.model_name_or_path
                )
                model.lm_head.load_state_dict(
                    pretrained_model.cls.predictions.state_dict()
            ...
            if model_args.ssl_type == "simcse":
                ...
                model_class = BertForCorInfoMax
            
            elif model_args.ssl_type == "vicreg":
                model_class = BertForVICReg
            elif model_args.ssl_type == "barlow":
                model_class = BertForBarlow
            else:
                raise ValueError("Unknown ssl_type")
            print(config)
            model = model_class.from_pretrained(
    ...
    def prepare_features_with_dropout(examples):
        ...
        if sent2_cname is not None:
            ...
            for key in sent_features:
                features[key] = [
                    [sent_features[key][i], sent_features[key][i]] for i in range(total)
                ]

        return features
    
    def prepare_features_with_corruption(examples):
        total = len(examples[sent0_cname])
        ...
        chars = ["a", "b", "c", "d", "e", "f", "g", "h", "ı", "j", "k", "l", "m", "n", "o", "p", "r", "s", "z"]
        initial_sentences = examples[sent0_cname]
        corrupted_sentences = []

        for sentence in initial_sentences:
            corrupted_sentence = sentence
            for i in range(len(sentence)//10):
                corrupted_sentence.replace(corrupted_sentence[random.randint(0, len(corrupted_sentence)-1)], chars[random.randint(0, len(chars)-1)])
            corrupted_sentences.append(corrupted_sentence)

        initial_sent_features = tokenizer(
            initial_sentences,
            max_length=data_args.max_seq_length,
            truncation=True,
            padding="max_length" if data_args.pad_to_max_length else False,
        ...
        corrupted_sent_features = tokenizer(
            corrupted_sentences,
            max_length=data_args.max_seq_length,
            truncation=True,
            padding="max_length" if data_args.pad_to_max_length else False,
        ...
        features = {}
        for key in initial_sent_features:
            features[key] = [
                 [initial_sent_features[key][i], corrupted_sent_features[key][i]] for i in range(total)
            ]

        return features

    def prepare_features_with_synonym_replacement(examples):
        total = len(examples[sent0_cname])
        ...
        for sentence in initial_sentences:
            word_to_replace = sentence.split()[random.randint(0, len(sentence.split())-1)]
            try:
                synonym = wn.synset(word_to_replace).lemma_names('eng')[0]
            except:
                synonym = word_to_replace
            corrupted_sentences.append(sentence.replace(word_to_replace, synonym))
    ...
    def prepare_features_with_random_swap(examples):
        total = len(examples[sent0_cname])
        # Avoid "None" fields
        for idx in range(total):
            if examples[sent0_cname][idx] is None:
        ...
        for sentence in initial_sentences:
            corrupted_sentence = sentence.split()

            rand_ind1 = random.randint(0, len(corrupted_sentence)-1)
            rand_ind2 = random.randint(0, len(corrupted_sentence)-1)

            word1 = corrupted_sentence[rand_ind1]
            word2 = corrupted_sentence[rand_ind2]

            corrupted_sentence[rand_ind1] = word2
            corrupted_sentence[rand_ind2] = word1

            corrupted_sentences.append(" ".join(corrupted_sentence))
    ...
    def prepare_features_with_random_delete(examples):
        total = len(examples[sent0_cname])
        ...
        for sentence in initial_sentences:
            ...
            for word in corrupted_sentence:
                if random.randint(0, 100) <= 15:
                    corrupted_sentence.remove(word)
    ...
    if training_args.do_train:
        if training_args.augmentation_method == "dropout":
            train_dataset = datasets["train"].map(
                prepare_features_with_dropout,
                batched=True,
                num_proc=data_args.preprocessing_num_workers,
                remove_columns=column_names,
                load_from_cache_file=not data_args.overwrite_cache,
            )
        elif training_args.augmentation_method == "corruption":
            train_dataset = datasets["train"].map(
                prepare_features_with_corruption,
                batched=True,
                num_proc=data_args.preprocessing_num_workers,
                remove_columns=column_names,
                load_from_cache_file=not data_args.overwrite_cache,
        ...
        elif training_args.augmentation_method == "synonym":
            train_dataset = datasets["train"].map(
                prepare_features_with_synonym_replacement,
                batched=True,
                num_proc=data_args.preprocessing_num_workers,
                remove_columns=column_names,
                load_from_cache_file=not data_args.overwrite_cache,
            )
        elif training_args.augmentation_method == "swap":
            train_dataset = datasets["train"].map(
                prepare_features_with_random_swap,
                batched=True,
                num_proc=data_args.preprocessing_num_workers,
                remove_columns=column_names,
                load_from_cache_file=not data_args.overwrite_cache,
        ...
        elif training_args.augmentation_method == "delete":
            train_dataset = datasets["train"].map(
                prepare_features_with_random_delete,
                batched=True,
                num_proc=data_args.preprocessing_num_workers,
                remove_columns=column_names,
                load_from_cache_file=not data_args.overwrite_cache,
