...
class MLPLayer(nn.Module):
    ...
    def __init__(self, training_args):
        super().__init__()
        sizes = [768] + list(map(int, training_args.proj_output_dim.split('-')))
        layers = []
...
class CLTrainer(Trainer):
    def evaluate(
        ...
        if (self.args.do_proj):
            mlp = MLPLayer(self.args)
        
        else:
            mlp = OneLayeredMLPLayer()
        ...
        def batcher(params, batch):
            ...
            with torch.no_grad():
                ...
                pooler_output = mlp(pooler_output.cpu())
        ...
        alignment = results["STSBenchmark"]["dev"]["alignment"]
        uniformity = results["STSBenchmark"]["dev"]["uniformity"]
        ...
        if (
            ...
            if isinstance(self.model, PreTrainedModel):
                self.model = self.model.from_pretrained(
                    self.state.best_model_checkpoint, training_args=self.args, model_args=self.model_args
                )
                if not self.is_model_parallel:
