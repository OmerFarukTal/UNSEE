...
def corinfomax_forward(
    ...
    save_results.save_values(writer, cls.cov_loss_fct, cls.training_args.cov_loss_weight, cls.training_args.sim_loss_weight, encoder.transformer.layer[-1].sa_layer_norm.weight,encoder.transformer.layer[-1].sa_layer_norm.bias)
...
def vicreg_forward(
    ...
    save_results.save_values(writer, cls.training_args.cov_coeff, cls.training_args.sim_coeff, cls.training_args.std_coeff,encoder.transformer.layer[-1].sa_layer_norm.weight,encoder.transformer.layer[-1].sa_layer_norm.bias)
    # Calculate loss for MLM
    if mlm_outputs is not None and mlm_labels is not None:
        loss_fct = nn.CrossEntropyLoss()
        mlm_labels = mlm_labels.view(-1, mlm_labels.size(-1))
...
def barlow_forward(
    ...
    save_results.save_values(writer,cls.training_args.lambd,encoder.transformer.layer[-1].sa_layer_norm.weight,encoder.transformer.layer[-1].sa_layer_norm.bias)
    # Calculate loss for MLM
    if mlm_outputs is not None and mlm_labels is not None:
        loss_fct = nn.CrossEntropyLoss()
        mlm_labels = mlm_labels.view(-1, mlm_labels.size(-1))
